  ...
  return new Promise((resolve, reject) => {
    db.serialize(async () => {
      try {
        ...
        const ledgerEntries = [
          [1, datetime('now'), 1, 'receipt', receipt1Id, 1, 15, 15, 'Initial laptop shipment'],
          [1, datetime('now'), 1, 'receipt', receipt1Id, 4, 10, 10, 'Initial laptop shipment'],
          [2, datetime('now'), 1, 'receipt', receipt2Id, 1, 100, 100, 'Office supplies delivery'],
          [2, datetime('now'), 1, 'receipt', receipt2Id, 1, 50, 50, 'Office supplies delivery'],
          [2, datetime('now'), 1, 'receipt', receipt2Id, 1, 20, 20, 'Office supplies delivery'],
          [2, datetime('now'), 1, 'delivery', delivery1Id, 1, -5, 40, 'ABC Corporation delivery'],
          [11, datetime('now'), 1, 'delivery', delivery1Id, 1, -10, 58, 'ABC Corporation delivery'],
          [13, datetime('now'), 1, 'delivery', delivery1Id, 1, -3, 19, 'ABC Corporation delivery']
        ];
        
        for (const [product_id, timestamp, user_id, type, ref_id, location_to, qty_change, balance_after, note] of ledgerEntries) {
          db.run(
            'INSERT OR IGNORE INTO stock_ledger (product_id, timestamp, user_id, type, ref_id, location_to, qty_change, balance_after, note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [product_id, timestamp, user_id, type, ref_id, location_to, qty_change, balance_after, note]
          );
        }
